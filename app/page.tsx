...
export default function HomePage() {
  ...
  const submitBooking = (event: React.FormEvent) => {
    ...
    const message = `السلام عليكم، أود حجز موعد.%0Aالاسم: ${form.fullName}%0Aالهاتف: ${form.phone}%0Aالتاريخ: ${form.date}%0Aالوقت: ${form.time}%0Aنوع المراجع: ${form.patientType === "new" ? "مريض جديد" : "مراجع سابق"}${form.purpose ? `%0Aالغرض: ${form.purpose}` : ""}`;
    setSent(true);

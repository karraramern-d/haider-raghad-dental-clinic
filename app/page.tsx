...
export default function HomePage() {
  ...
  const [form, setForm] = useState({ fullName: "", phone: "", date: new Date().toISOString().slice(0, 10), time: "", patientType: "new", sessionNumber: "", purpose: "" });
  const [availability, setAvailability] = useState<Record<string,string>>({});
  const [tilt, setTilt] = useState({ x: 0, y: 0 });
  const minDate = useMemo(() => new Date().toISOString().slice(0, 10), []);
  const bookingDates = useMemo(() => Array.from({ length: 5 }, (_, index) => {
    const date = new Date(`${minDate}T12:00:00`);
    date.setDate(date.getDate() + index);
    return { value: date.toISOString().slice(0, 10), day: date.toLocaleDateString("ar-IQ", { weekday: "short" }), number: date.getDate(), month: date.toLocaleDateString("ar-IQ", { month: "short" }) };
  }), [minDate]);
  ...
  const submitBooking = async (event: React.FormEvent) => {
    ...
    if (!form.time) { window.alert("اختر وقت الموعد أولًا"); return; }
...
الغرض من الجلسة: ${form.purpose}` : ""}`;
  ...
  const renderSlot = (slot: typeof slots[number]) => {
    ...
    const booked = Boolean(status && status !== "available");
    const selected = form.time === slot.value;
    return <button type="button" key={slot.value} className={`slot-card ${booked ? "booked" : "available"} ${selected ? "selected" : ""}`} disabled={booked} aria-pressed={selected} onClick={() => setForm({ ...form, time: slot.value })}><strong dir="ltr">{slot.value}</strong><span>{booked ? "غير متاح" : "متاح"}</span></button>;
  };

  const bookingDialog = <div className="booking-overlay" onMouseDown={(e) => { if (e.currentTarget === e.target) setBookingOpen(false); }}><section className="booking-modal" role="dialog" aria-modal="true" aria-labelledby="booking-title"><button className="close-button" onClick={() => setBookingOpen(false)} aria-label="إغلاق"><Icon name="close" /></button>{sent ? <div className="booking-success"><div className="success-icon"><Icon name="check" size={32} /></div><span className="eyebrow">تم إرسال الطلب</span><h2>شكرًا، وصلنا طلبك</h2><p>سيتم فتح واتساب لإكمال التواصل مع العيادة وتأكيد الموعد.</p><button className="primary-button" onClick={() => { setSent(false); setBookingOpen(false); }}>العودة للموقع</button></div> : <><span className="eyebrow booking-kicker">حجز موعد</span><h2 id="booking-title">اختر ما يناسبك</h2><p className="modal-intro">املأ البيانات الأساسية، ثم أرسل طلبك عبر واتساب.</p><div className="booking-steps"><span className="active"><b>1</b><small>بياناتك</small></span><i /><span><b>2</b><small>الموعد</small></span><i /><span><b>3</b><small>التأكيد</small></span></div><form onSubmit={submitBooking}><label>الاسم الكامل<input required value={form.fullName} onChange={(e) => setForm({ ...form, fullName: e.target.value })} placeholder="اكتب اسمك الكامل" /></label><label>رقم الهاتف<input required inputMode="tel" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} placeholder="0000 000 0770" /></label><div className="booking-heading"><span><Icon name="calendar" size={21} /> اختر التاريخ</span></div><div className="booking-date-row"><button type="button" aria-label="الأيام السابقة" className="date-arrow" onClick={() => {}}><Icon name="arrow" size={16} /></button><div className="booking-dates" aria-label="اختيار التاريخ">{bookingDates.map((date) => <button type="button" key={date.value} className={form.date === date.value ? "selected" : ""} onClick={() => setForm({ ...form, date: date.value, time: "" })}><small>{date.day}</small><strong>{date.number}</strong><small>{date.month}</small></button>)}</div><button type="button" aria-label="الأيام التالية" className="date-arrow" onClick={() => {}}><Icon name="arrow" size={16} /></button></div><input className="visually-hidden" required type="date" min={minDate} value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} tabIndex={-1} /><div className="booking-heading"><span><Icon name="clock" size={21} /> اختر الوقت</span></div><div className="time-period"><span>الفترة الصباحية</span><div className="slot-grid" role="radiogroup" aria-label="الأوقات الصباحية">{slots.filter((slot) => Number(slot.value.slice(0, 2)) < 13).map(renderSlot)}</div></div><div className="time-period"><span>الفترة المسائية</span><div className="slot-grid" role="radiogroup" aria-label="الأوقات المسائية">{slots.filter((slot) => Number(slot.value.slice(0, 2)) >= 16).map(renderSlot)}</div></div><label>نوع المراجع<select value={form.patientType} onChange={(e) => setForm({ ...form, patientType: e.target.value, sessionNumber: e.target.value === "new" ? "" : form.sessionNumber })}><option value="new">مريض جديد</option><option value="returning">مراجع سابق</option></select></label>{form.patientType === "returning" && <label>رقم الجلسة القادمة<select required value={form.sessionNumber} onChange={(e) => setForm({ ...form, sessionNumber: e.target.value })}><option value="">اختر رقم الجلسة</option>{Array.from({ length: 9 }, (_, index) => index + 2).map((number) => <option key={number} value={number}>الجلسة {number === 2 ? "الثانية" : number === 3 ? "الثالثة" : number === 4 ? "الرابعة" : number === 5 ? "الخامسة" : number === 6 ? "السادسة" : number === 7 ? "السابعة" : number === 8 ? "الثامنة" : number === 9 ? "التاسعة" : "العاشرة"}</option>)}</select></label>}<label>الغرض من الجلسة <span className="optional">اختياري</span><input value={form.purpose} onChange={(e) => setForm({ ...form, purpose: e.target.value })} placeholder="مثال: فحص، تنظيف، ألم" /></label><div className="booking-summary"><span><Icon name="calendar" size={19} /> {form.date || "اختر التاريخ"}</span><span><Icon name="clock" size={19} /> {form.time || "اختر الوقت"}</span></div><button className="primary-button form-submit" type="submit">متابعة <Icon name="arrow" size={18} /></button></form></>}</section></div>;
  ...
  return <main className="public-shell">
    ...
    <section id="appointment" className="appointment section-wrap"><div className="appointment-copy"><span className="eyebrow">خطوتك الأولى</span><h2>موعدك يبدأ<br /><em>برسالة بسيطة</em></h2><p>اختر اليوم والوقت المناسبين، وسنرسل طلب الموعد مباشرة إلى واتساب العيادة للتأكيد.</p></div></section>

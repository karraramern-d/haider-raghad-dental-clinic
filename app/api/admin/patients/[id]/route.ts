...
export async function DELETE(_: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await context.params;
  try {
    return NextResponse.json(await supabaseRpc("admin_delete_test_patient", { p_session_token: token, p_patient_id: id }));
  } catch (error) {
    const message = error instanceof Error ? error.message : "تعذر حذف المراجع التجريبي";
